...
const ( // const后面是小括号
	Ldate = 1 << iota // 递增模式是 1<< n(0,1,2,...,位操作) 注意iota与itoa相区分
	Ltime
	Lmicroseconds
	Llongfile  // 绝对路径
	Lshortfile // 当前文件名称ß
...
type Logger struct { // struct后面大括号
...
func New(out io.Writer, prefix string, flag int) *Logger { //注意这里的大小写
...
func (l *Logger) SetOutput(w io.Writer) {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.out = w // 一个Logger对象在生命周期内任意特定时刻，只有一个输出终端
}
	...
	*buf = append(*buf, l.prefix...) //如果设置有前缀
	if l.flag&(Ldate|Ltime|Lmicroseconds) != 0 {
		...
		if l.flag&Ldate != 0 { //输出形如 “2015/06/13 "
			...
			itoa(buf, year, 4)
			*buf = append(*buf, '/')
			itoa(buf, int(month), 2) //Month类型是Int的 type转换过来的，需要强转
			*buf = append(*buf, '/')
			itoa(buf, day, 2)
			*buf = append(*buf, ' ')
		}
